...
const LiveVisitorFeed = ({ visitors = [] }) => {
  ...
  const getStatusColor = (status) => {
    ...
    if (statusLower.includes("check out") || statusLower.includes("checked out")) {
      return "bg-green-100 text-green-700 border border-green-300"; // Green for check-out
    }
    return "bg-yellow-100 text-yellow-700 border border-yellow-300"; // Yellow for check-in (default)
  };

  return (
    <section className="bg-white p-6 rounded-2xl border border-[#7400EA] shadow-sm mt-8 overflow-x-auto dark:bg-gray-900 dark:text-gray-200">
      {/* Header */}
      <div className="mb-4">
            ...
            visitors.map((v, i) => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  <span className={`font-semibold px-3 py-1 rounded-full text-xs sm:text-sm ${getStatusColor(v.status)}`}>
                    {v.status}
                  </span>
                </td>
              </tr>
